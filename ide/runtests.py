...
def dispatch(
    ...
    if not file_has_main(fname) and bname.startswith("test_"):
        cmd = "PYTHONPATH=.. CUDA_VISIBLE_DEVICES='' pytest {pytest_args} {fname}".format(
            **vars())
    elif fname.endswith(".py") and bname != 'conftest.py':
        cmd = "PYTHONPATH=.. python " + fname
    else:
        cmd = "PYTHONPATH=.. CUDA_VISIBLE_DEVICES='' pytest {pytest_args}".format(
            **vars())
