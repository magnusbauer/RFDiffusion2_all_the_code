...
p.add_argument('datadir', help='folder of pdbs to process')
...
for fn in filenames:
    pdb = parse_pdb(fn)
    ch = np.array([x[0] for x in pdb['pdb_idx']])

    seq = ''
    for ch_idx in np.unique(ch):
        if seq != '':
            seq = seq + '/'
        seq = seq+''.join([aa_N_1[i] for i in pdb['seq'][ch==ch_idx]])

    name = os.path.basename(fn).replace('.pdb','')
    outfn = args.outdir+'/'+name+'.fa'
