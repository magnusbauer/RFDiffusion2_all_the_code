...
def rigid_frames_from_atom_14(atom_14):
    '''
    Construct frames from C-Ca-N, then flip around y axis by pi.
    '''
    n_atoms = atom_14[..., 2,:]
    ca_atoms = atom_14[..., 1,:]
    c_atoms = atom_14[..., 0,:]
    ...
    rots = rigid_utils.Rotation(rot_mats=torch.tensor([[
            [-1, 0, 0],
            [0, 1, 0,],
            [0, 0, -1.]]]).to(rigids.device))
    # from icecream import ic
    # ic(rots.get_rotvec()) [0,  3.1416, 0]

    rigids = rigids.compose(rigid_utils.Rigid(rots, None))
    return rigids
