...
class SO3Diffuser:
    ...
    def t_from_sigma(self, sigma: np.ndarray):
        return (np.exp(sigma) - np.exp(self.min_sigma)) / (np.exp(self.max_sigma) - np.exp(self.min_sigma))
    ...
    def torch_score(
        ...
        Args:
            vec: [..., 3] : array of axis-angle rotation vectors.
            t: [1] continuous time in [0, 1].

        Returns:
            [1,..., 3] score vector in the direction of the sampled vector with
            magnitude given by _score_norms.
        """
        assert t.shape == (1,) ,f'{t.shape=}'
        ...
        if self.use_cached_score:
            ...
            score_norms_t = torch.tensor(score_norms_t).to(vec.device) # [1, 1000]
            score_norms_t = score_norms_t[0] # [1000]
            omega_idx = torch.bucketize(
                omega, torch.tensor(self.discrete_omega[:-1]).to(vec.device)) # [...]
            score_norms = score_norms_t[omega_idx] # [...]
            omega_scores_t = score_norms[None,...]
        else:
