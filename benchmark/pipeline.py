...
def main(conf: HydraConfig) -> None:
    ...
    if conf.start_step == 'sweep':
        ...
        jobid_sweep = run_pipeline_step(f'{script}  {hydra_overrides(conf.sweep)}')
    ...
    if conf.start_step in ['sweep', 'foldseek']:
        ...
        jobid_cluster = run_pipeline_step(f'{script_dir}/cluster_pipeline_outputs.py {hydra_overrides(conf.cluster)}')
        ...
        jobid_foldseek = run_pipeline_step(f'{script_dir}/chunkify_foldseek_pdb.py {hydra_overrides(conf.foldseek)}')
    ...
    if conf.start_step in ['sweep', 'foldseek', 'mpnn']:
        ...
        jobid_mpnn = run_pipeline_step(f'{script_dir}mpnn_designs.py {hydra_overrides(conf.mpnn)}')
